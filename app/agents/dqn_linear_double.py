...
class DDQNLinearAgent(pl.LightningModule):
    """An double deep-Q-network agent with a linear neural network structure."""

    name: Final[str] = "double_dqn_linear"
    ...
    def _make_model(
        ...
        model = nn.Sequential(
            nn.Flatten(),
            nn.Linear(x_dim * y_dim, 64),  # fully connected layer
            nn.ReLU(),
            nn.Linear(64, 16),
            nn.ReLU(),
